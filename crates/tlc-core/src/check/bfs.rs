...
/// Injected stop conditions (the wasm/CLI deadline and state cap). The
/// deadline callback is polled about every 4096 generated states.
...
impl Default for CheckBudget<'_> {
    fn default() -> Self {
        CheckBudget { deadline_exceeded: None, max_states: None, eval_stack_bytes: None }
    }
}
...
pub enum Outcome {
    ...
    /// Budget exhausted (deadline or state cap).
